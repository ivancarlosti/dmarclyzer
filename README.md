...
1. **Configure Environment:**
   Navigate into the `docker` directory and adjust the environment variables:
   ```bash
   cd docker
   # Edit .env with your real IMAP credentials, DB passwords, and target Port
   ```
...
3. **Access the Dashboard:**
   Navigate your web browser to `http://localhost:8080` (or whichever `PORT` you configured in your `.env`).
...
To set it up:
1. Ensure the `PORT` in your `.env` accurately maps to your reverse proxy's upstream route (the container internally exposes `8080`).
2. Utilize the `DOMAIN` variable within `.env` to formally set your URL for proxy routing references (like Nginx Proxy Manager configurations).

---

## Authentication Options

You can secure your Dashboard by editing the `AUTH_METHOD` variable in your `.env` file:

- **none** (Default): Disables authentication entirely.
- **account**: Enables a simple username/password login prompt. Requires you to configure `ACCOUNT_LOGIN` and `ACCOUNT_PASSWORD`. ReCaptcha validation variables are also available.
- **keycloak**: Enables Single Sign-On using Keycloak. Requires you to specify your `KEYCLOAK_BASE_URL`, `KEYCLOAK_REALM`, `KEYCLOAK_CLIENT_ID`, `KEYCLOAK_CLIENT_SECRET`, and `KEYCLOAK_REDIRECT_URI`. You can also restrict access to a specific email with `KEYCLOAK_EMAIL_ACCOUNT`.
